...
public class Entity extends GameObject {
	...
	protected void collision() {
		for(int i = 0;i < m.getGs().getSolidObjects().size();i++) {
			SolidObjects bl =  m.getGs().getSolidObjects().get(i);
...
//				else
				...
				if(bl.getType()==1) {
					...
					if(bl.getX()== 416&&bl.getY()==1408) {//specific block has muschroom
						m.getGs().getItems().add(new PowerUp(m,bl.getX(),bl.getY()-40,32,32));
					}else {
						m.getGs().getUi().coins++;
					}
...
//				System.out.println(bl.y);
	...
	private void entityCollision() {
		for(int i = 0;i<m.getGs().getEntities().size();i++) {
			Entity e = m.getGs().getEntities().get(i);
